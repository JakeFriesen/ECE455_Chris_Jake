...
//    dd_task *pTaskParameters = pvPortMalloc(sizeof(dd_task));
	...
	dd_task pTaskParameters = {.type = periodic,
							   .execution_time = 100,
							   .absolute_deadline = xTaskGetTickCount() - START + relative_deadline,
							   .release_time = 0,
							   .completion_time = 0,
							   .t_handle = NULL };
...
//        {
            ...
            if (periodic_task_message.task_ptr->type == periodic)
            {
                ...
                int startTime = pdMS_TO_TICKS(pTaskParameters.absolute_deadline);
                ...
                if (dd_create(&pTaskParameters, "PERIODIC_TASK") == pdFAIL)
                    printf("dd_tcreate Failed!\n");
//                	while(1);
            }
//        }
...
static void Auxiliary_Task(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        // delete the task!
        printf("AET %d\n", xTaskGetTickCount());
        if(dd_delete(xTaskGetCurrentTaskHandle()) == pdPASS){
...
static BaseType_t dd_create(dd_task * task_data, const char *const task_name)
{
    printf("dd_tcreate. Name: %s, handle: %s\n", task_name, (char*)task_data->t_handle);
    BaseType_t response = pdFAIL;
